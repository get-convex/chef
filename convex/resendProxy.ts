...
export const decrementToken = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        const token = await ctx.db.query('resendTokens')
        ...
        if (token.verifiedEmail !== args.recipientEmail) {
            return { success: false, error: `The Convex Resend API Proxy only supports sending email to your own verified email address (${token.verifiedEmail}).` };
        }
...
export const issueResendToken = mutation({
    handler: async (ctx) => {
        ...
        const existing = await ctx.db.query('resendTokens')
