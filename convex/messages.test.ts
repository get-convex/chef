...
// test('sending message after rewind preserves storageId referenced by previous chatMessageStorageState', async () => {
//   vi.useFakeTimers();
//   const t = setupTest();
//   const { sessionId, chatId } = await createChat(t);
...
//   // Get the first storage state to verify its storage blob later
//   const firstStorageInfo = await t.query(internal.messages.getInitialMessagesStorageInfo, {
//     sessionId,
//     chatId,
//   });
//   expect(firstStorageInfo).not.toBeNull();
//   expect(firstStorageInfo?.storageId).not.toBeNull();
//   const sharedStorageId = firstStorageInfo?.storageId;
//   if (!sharedStorageId) throw new Error('No storage ID');

//   // Call storeChat without new message content
//   await storeChat(t, chatId, sessionId, {
//     messages: [firstMessage, firstMessage],
//     doNotUpdateMessages: true,
//     snapshot: new Blob(['hello']),
//   });
...
//   });
//   // Should have: initialize chat, first message, and new message states
//   console.log('storageStates', storageStates);
...
//   // Verify the original storage blob still exists and contains the first message
//   await t.run(async (ctx) => {
//     const sharedStorageBlob = await ctx.storage.get(sharedStorageId);
//     expect(sharedStorageBlob).not.toBeNull();
//     if (!sharedStorageBlob) throw new Error('Shared storage blob was deleted');
//     const content = await sharedStorageBlob.text();
//     expect(JSON.parse(content)).toEqual([firstMessage]);
//   });
...
//       .collect();
//   });
//   console.log('finalStorageStates', finalStorageStates);
//   // Should have: initialize chat, first message, and new message states
//   expect(finalStorageStates.length).toBe(3);

//   // Verify the newest message has the new content
//   const newestMessage = finalStorageStates[2];
//   if (!newestMessage?.storageId) throw new Error('No storage ID');
//   await verifyStoredContent(t, newestMessage.storageId, JSON.stringify([firstMessage, newMessage]));
