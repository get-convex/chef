...
const readRangeDescription = `
An optional array of two numbers specifying the inclusive start and exclusive end line numbers to read.
...
const fileReadContentsDescription = `
...
export const fileReadContentsParameters = z.object({
  absolute_path: z.string().describe('The absolute path to the file to read.'),
  read_range: z.array(z.number()).optional().describe(readRangeDescription),
});

export const fileReadContentsTool: Tool = {
  description: fileReadContentsDescription,
  parameters: fileReadContentsParameters,
};
