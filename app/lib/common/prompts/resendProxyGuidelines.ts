...
export function resendProxyGuidelines(options: SystemPromptOptions) {
  ...
  <bundled_resend_proxy_guidelines>
    Apps in the Chef environment come with a small amount emails to
    send via the Resend API! The Chef environment ONLY supports
    ...
    Convex Resend proxy to send emails to test@example.com.
    ...
    export const sendReport = action({
      ...
      handler: async (ctx, args) => {
        ...
        const { data, error } = await resend.emails.send({
          from: "Chef <thechef@convexchef.app>",
          to: args.to,
